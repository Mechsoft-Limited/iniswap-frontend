...
import { Modal, Text, Flex, BalanceInput, Box, Button, PancakeRoundIcon } from '@iniswap/uikit'
...
const PlaceBidModal: React.FC<PlaceBidModalProps> = ({
  ...
  const { balance: userCake, fetchStatus } = useTokenBalance(tokens.ini.address)
  ...
  useEffect(() => {
    if (userNotEnoughCake) {
      setErrorText(t('Insufficient INI balance'))
    } else if (!isMoreThanInitialBidAmount && isFirstBid) {
      setErrorText(t('First bid must be %initialBidAmount% INI or more.', { initialBidAmount }))
    ...
    <StyledModal title={t('Place a Bid')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      <ExistingInfo>
        <Flex justifyContent="space-between">
          ...
          <Text>{t('%num% INI', { num: getBalanceNumber(amount).toLocaleString() })}</Text>
        </Flex>
        <Flex justifyContent="space-between">
      ...
      <InnerContent>
        ...
        {isFirstBid && (
          <Text pb="8px" small>
            {t('First bid must be %initialBidAmount% INI or more.', { initialBidAmount })}
          </Text>
        )}
        <BalanceInput
        ...
        <Text color="textSubtle" small mt="24px">
          {t('If your bid is unsuccessful, you’ll be able to reclaim your INI after the auction.')}
        </Text>
      </InnerContent>
    </StyledModal>
  )
